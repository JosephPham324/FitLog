...
public class MuscleGroups : EndpointGroupBase
{
    ...
    public Task<UpdateMuscleGroupDTO> UpdateMuscleGroup(ISender sender, [FromBody] UpdateMuscleGroupCommand command)
    {
        return sender.Send(command);
    }

    public Task<bool> DeleteMuscleGroup(ISender sender, [FromBody] DeleteMuscleGroupCommand command)
    {
        return sender.Send(command);
    }
}
